...
void assign_op(quad instr){
    if(is_integer(instr.arg1.first)){
        ...
        code_file << "\tmov "<< mem << ", "<< stoi(instr.arg1.first) <<endl;
    }
}
...
void genCode(){
    ...
    for (auto it : leaders)cout<<it<<endl;
...
void nextUse(){
    for(auto it=leaders.begin(); it != leaders.end(); it++){
        ...
        for(int i= (*it1)-1; i>= (*it); i--){
            if(code[i].arg1.first != "" && code[i].arg1.second && code[i].arg1.second->next_use == -1){
                code[i].arg1.second->next_use = i;
            } 
            if(code[i].arg2.first != "" && code[i].arg1.second && code[i].arg2.second->next_use == -1){
